...
为老人端数字人提供日程管理、情绪记录、媒体展示、家属联系等功能的 MCP 工具服务器。

**特点：一对一关系，无需传递 family_id 和 elderly_id！**

## 核心特性

- ✅ **简化设计**: 老人端与家属端一对一关系，自动管理用户ID
- ✅ **API集成**: 直接调用 KinEcho 服务端API，无需直接操作数据库
- ✅ **环境配置**: 通过环境变量配置 family_id，适配不同部署环境
- ✅ **10个工具**: 覆盖联系家属、情绪记录、媒体播放、日程管理
...
export KINECHO_API_URL="http://127.0.0.1:8000"

# 家庭组ID（默认: family_001）
export KINECHO_FAMILY_ID="family_001"

# 老人用户ID（默认: 1）
export KINECHO_ELDERLY_ID="1"
```
...
```json
{
  "mcpServers": {
    "kinecho-elderly": {
      "command": "python",
      "args": ["d:/Projects/KinEcho/elderly_mcp/server.py"],
      "env": {
        "KINECHO_API_URL": "http://127.0.0.1:8000",
        "KINECHO_FAMILY_ID": "family_001",
        "KINECHO_ELDERLY_ID": "1"
      }
    }
  }
}
...
- **情绪记录**: MCP → API (`/api/elderly/moods`) → 数据库 → 家属端查询
- **联系家属**: MCP → API (`/api/family/alerts`) → 数据库 → 家属端实时告警
- **媒体播放**: MCP → API (`/api/media/display`) → 记录播放历史 → 家属端统计
- **日程管理**: MCP ← API (`/api/elderly/schedules`) ← 数据库 ← 家属端创建

## 与服务端API对接

本MCP服务器依赖 KinEcho 服务端提供的以下API:

| API端点 | 方法 | 用途 | 状态 |
|---------|------|------|------|
| `/api/family/alerts` | POST | 发送告警给家属 | ✅ 已实现 |
...
| `/api/family/media` | GET | 获取媒体列表 | ✅ 已实现 |
| `/api/elderly/media/{id}/play` | POST | 播放媒体 | ✅ 已实现 |
| `/api/family/schedules` | GET | 获取日程列表 | ✅ 已实现 |
...
解决:
1. 检查环境变量 `KINECHO_FAMILY_ID` 的值
2. 确保家属端查询时使用相同的 family_id
3. 查看服务端日志确认记录是否写入
...
1. **环境变量**（推荐）:
   ```bash
   export KINECHO_FAMILY_ID="my_family"
   ```
