...
```json
{
  "success": true,
  "elderly_users": [
    {"id": 1, "name": "张奶奶", "family_id": "family_001", "user_type": "elderly"}
  ],
...
- 负面情绪（sad/anxious/angry 或分数≤3）会额外发送告警到家属端
- 家属可通过以下API查看：
...
- 会创建 `media_display` 类型的事件用于通知老人端界面
- 家属可查看播放统计和老人的观看偏好

### 日程管理
- 日程状态变更会同步到 `schedules` 表
...
2. 确认日程的 `elderly_id` 与查询的ID一致
3. 使用 `get_schedules` 查看所有日程
