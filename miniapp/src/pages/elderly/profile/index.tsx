  ...
  const loadData = useCallback(async () => {
    try {
      const [nextUsers, records, mediaHistory] = await Promise.all([
        ...
        getMoodRecords(DEFAULT_FAMILY_ID, DEFAULT_ELDERLY_ID, 60),
        getMediaHistory(DEFAULT_ELDERLY_ID, 60),
      ]);
      setUsers(nextUsers);
  ...
  const settingGroups = useMemo(
    () => [
      ...
      },
      {
        title: '陪伴设置',
        items: [
          { icon: '心', label: '数字人设置', value: '小心·温柔风格' },
    ...
    [elderly, familyMembers, history.length, moodRecords.length]
  );
    ...
    <View className='ef-page ef-page--tab'>
      <View className='ef-profile-hero'>
        <View className='ef-profile-avatar'>
          <Text>{elderly?.name?.slice(0, 1) || '张'}</Text>
        </View>
        <View>
          <Text className='ef-profile-name'>{elderly?.name || '张翠花'}</Text>
      ...
      <View className='ef-profile-groups'>
        ...
        <Button className='ef-logout-button'>退出登录</Button>

        <View className='ef-version'>
          <Text>老年心理健康陪护平台</Text>
          <Text>数据由 MySQL 管理</Text>
        </View>
      </View>
